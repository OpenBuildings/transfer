...
class InitegrationTest extends AbstractTestCase
{
    ...
    public function testTest()
    {
        ...
        $this->assertQueries([
           ...
           'UPDATE Item SET transferId = CASE id WHEN 3 THEN "2" WHEN 4 THEN "2" ELSE transferId END, refId = CASE id WHEN 3 THEN 1 WHEN 4 THEN 2 ELSE refId END WHERE (id IN (3, 4))',
           'SELECT Basket.* FROM Basket WHERE (id IN ("2")) AND (Basket.deletedAt IS NULL)',
           'SELECT Basket.* FROM Basket WHERE (id IN ("2")) AND (Basket.deletedAt IS NULL)',
           'UPDATE Basket SET isSuccessful = 1, completedAt = "'.$basket->completedAt.'", responseData = "{"amount":"1000.00","reference":"'.$basket->responseData['reference'].'","success":true,"message":"Success"}", isFrozen = 1, value = 100000 WHERE (id = "2")',
           'UPDATE Item SET isFrozen = CASE id WHEN 3 THEN 1 WHEN 4 THEN 1 ELSE isFrozen END, value = CASE id WHEN 3 THEN 10000 WHEN 4 THEN 20000 ELSE value END WHERE (id IN (3, 4))',
        ]);
    }
}
